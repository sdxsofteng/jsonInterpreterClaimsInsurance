...
public class Validation {
    ...
    public static void ValidateInvoice(Customer customer, CareReference referenceObject) {
        if (isValidClientNo(customer.getClientNumber())
                && isValidContractType(customer.getContractLetter(), referenceObject)
                && isValidInvoiceDate(customer.getClaimPeriod())
                && validateAllClaims(customer.getClaimsList(), customer.getClaimPeriod(), referenceObject)){
    ...
    public static boolean isValidContractType(String contractType, CareReference referenceObject){
        boolean isValid = false;
        List<String> contractTypesList = referenceObject.getContractTypesList();
        for (String type: contractTypesList) {
            if (contractType.trim().equals(type)) {
                isValid = true;
            }
        }
        return isValid;
    ...
    public static boolean isValidInvoiceDate(String invoiceDate){
        boolean isValid = (isValidDateFormatYM(invoiceDate) && isThisMonthOrEarlier(invoiceDate));
        return isValid;
    }
    ...
    public static boolean validateAllClaims(List<Claims> claimList, String claimPeriod, CareReference referenceObject){
